...
> **C# LSP**: wpf-dev-pack은 이제 자체 `.lsp.json`(C# 언어 서버 설정)을 번들로 포함하므로
> 별도 `csharp-lsp` 플러그인이 더 이상 필요하지 않습니다. PATH에 `csharp-ls` 바이너리만 있으면
> 됩니다 — 아래 **C# LSP (IntelliSense용 필수)** 섹션을 참고하세요.
...
| _(번들 `.lsp.json`)_ | csharp (`csharp-ls`) | C# LSP 코드 인텔리전스 |
...
wpf-dev-pack은 C# 언어 서버를 설정하는 `.lsp.json`을 번들로 포함하므로 별도 LSP 플러그인이
필요 없습니다. `csharp-ls` 바이너리(.NET 글로벌 도구)를 설치하세요:

```bash
dotnet tool install -g csharp-ls
```

`csharp-ls`가 PATH에 없으면 Claude Code의 `/plugin` Errors 탭에 `Executable not found in $PATH`가
표시되며, LSP 코드 인텔리전스 없이 나머지 플러그인 기능은 정상 동작합니다.
