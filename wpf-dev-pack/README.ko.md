...
**62개 스킬** · **11개 전문 에이전트** · **5개 명령어** · **2개 MCP 서버**
...
- **.NET 어셈블리 리플렉션**을 위한 **HandMirror** (외부)
...
| **HandMirror** | .NET 어셈블리 리플렉션 & NuGet 검사 | 아키텍처 & 리뷰 에이전트 |
...
| **HandMirror** | .NET 어셈블리 리플렉션 | 별도 설치 |
...
├── 📁 skills/                 # 62개 스킬
├── 📁 hooks/                  # 이벤트 훅
├── 📄 .mcp.json               # MCP 설정 (MicrosoftDocs, HandMirror)
├── 📄 README.md
└── 📄 LICENSE
```
